			...
			<el-card shadow="always" class="box-card">
				<el-tabs v-model="activeName" @tab-click="handleClick">
					<el-tab-pane label="发布" name="first">
						<div class="serach">
							<el-button @click="showTree = !showTree" type="primary" style="margin-left: 16px;">
							  筛选
							</el-button>
							<div>
								<el-button 
							...
							<ul>
								<li>
									<el-input 
										v-model="inputSerach" 
										placeholder="搜索关键词" 
										autosize clearable></el-input></li>
								<li>
									<el-button 
										type="primary" 
										icon="el-icon-search" 
								 ...
								 max-height="580">
									<el-table-column prop="head" label="封面" width="180" align="center">
										<template slot-scope="scope">
											<div class="block">
												<el-avatar shape="square" :size="100" :src="scope.row.imagePath"></el-avatar>
											</div>
...
export default {
	...
	methods: {
		//添加新闻
		addNews(){
			bus.$emit('getNewsCurrentInfo','')
			this.$router.push('/home/mnewseditor')
		},
		//移动栏目提交
		moveSubmit(){
			let p={
			...
			moveNewsChannelAPI(p).then(res=>{
				console.log('移动ok',res);
				this.getNewsList('','1',1,10,1)
				this.isCopy=false
			})
		},
		//获取新闻列表
		getNewsList(searchContent,channelId,audit ,pageSize,pageNumber){
			let p={
		...
		handleSubmit(){
			...
			console.log(p);
			approveNewsAPI(p).then(res=>{
				console.log('审核结果',res);
				if(res.status==1){
					this.getNewsList('','',0,10,1)
		...
		getCurrentPage(val){
			...
			let index=val
			this.loading=true
			this.getNewsList('',this.currentColumnId,'1',10,index)
		},
		//获取下一页
		getApprovCurrentPage(val){
			...
			let index=val
			this.loading=true
			this.getNewsList('','',this.tabradio,10,index)
		},
		// 预览
		handlePreView() {
			console.log('preview');
		...
		onReadNumberSubmit(){
			...
			modifyVmReadNumberAPI(p).then(res=>{
				if(res.status==1){
					console.log('修改浏览量结果',res);
					this.getNewsList('','',1,10,1)
		...
		handleClick(tab, event) {
			...
			if(tab.index=='0'){
				...
				this.tabradio="0"
				this.getNewsList('','',this.tabradio,10,1)
			}
		},
		tableRowClassName() {
		...
		handleDel(id) {
			...
			this.$confirm('确定删除该新闻, 是否继续?', '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			})
				.then(() => {
					deleteNewsByIdAPI(params).then(res => {
						if (res.status == 1) {
							this.$message({
								type: 'success',
								message: '删除成功!'
							});
							this.getNewsList('',this.currentColumnId,1,10,1)
						}
					});
				})
				.catch(() => {
		...
		handleEdi(id) {
			console.log("新闻id",id);
			let p={id:id}
			getNewsDetailAPI(p).then(res=>{
				console.log('新闻详情',res);
				bus.$emit('getNewsCurrentInfo',res)
				this.$router.push('/home/mnewseditor')
			})
		},
		// 修改浏览量
